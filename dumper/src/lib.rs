...
fn dump_inner<M: Mem + Clone>(
    ...
    let guobjectarray = ExternalPtr::<FUObjectArray>::try_new(results.guobject_array.0)?;
...
fn read_object<M: Mem + Clone>(
    ...
    fn read_script_struct<M: MemComplete>(obj: &CtxPtr<UScriptStruct, M>) -> Result<ScriptStruct> {
        Ok(ScriptStruct {
