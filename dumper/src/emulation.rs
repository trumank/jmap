...
impl<M: Mem> MemoryTrait for MemoryAdapter<M> {
    fn find_region(&self, _addr: u64) -> Option<remu64::memory::MemoryRegionRef<'_>> {
        None
    }
    fn find_region_mut(&mut self, _addr: u64) -> Option<remu64::memory::MemoryRegionMut<'_>> {
        None
    }
    fn read(&self, addr: u64, buf: &mut [u8]) -> Result<()> {
        self.mem.read_buf(addr, buf).map_err(|e| {
